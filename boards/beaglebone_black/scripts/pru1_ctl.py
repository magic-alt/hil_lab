...
DEFAULT_FIRMWARE_NAME = "am335x-pru1-fw"
PRU1_NAMES = ("4a338000.pru", "4a338000.pru1", "pruss-core1")
...
def find_pru1_remoteproc(explicit: str | None = None) -> Path:
    if explicit:
        path = Path(explicit)
        if not path.exists():
    ...
    for raw in sorted(glob.glob("/sys/class/remoteproc/remoteproc*")):
        ...
        if (
            name in PRU1_NAMES
            or "4a338000.pru" in lower
            or "pruss-core1" in lower
        ):
            matches.append(path)
    ...
    if len(matches) != 1:
        ...
        for raw in sorted(glob.glob("/sys/class/remoteproc/remoteproc*")):
            ...
            try:
                ...
                pass
        raise RuntimeError(
            f"could not uniquely identify PRU1 remoteproc; found {found}"
        )
    return matches[0]
...
def wait_for_rpmsg(timeout_s: float) -> str:
    ...
    patterns = ("/dev/rpmsg_pru31", "/dev/rpmsg-pru31", "/dev/rpmsg*31*")
        ...
        for pattern in patterns:
            for candidate in glob.glob(pattern):
                if Path(candidate).exists():
                    return candidate
        time.sleep(0.05)
    raise TimeoutError("RPMsg PRU1 character device did not appear")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Manage BBB PRU1 remoteproc for hil_lab B2"
    )
    parser.add_argument(
        "--remoteproc",
        help="override /sys/class/remoteproc/remoteprocN",
    )
    ...
    remoteproc = find_pru1_remoteproc(args.remoteproc)
